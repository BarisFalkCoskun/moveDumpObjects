export function cleanNettoEAN(element) {
  delete element.multipromos;

  // Clean storeData
  if (element?.storeData) {
    // Process each store object
    for (const storeId in element.storeData) {
      const storeObj = element.storeData[storeId];

      // Remove price and unitsOfMeasurePrice if they are 0
      if (storeObj.price === 0) {
        delete storeObj.price;
      }
      if (storeObj.unitsOfMeasurePrice === 0) {
        delete storeObj.unitsOfMeasurePrice;
      }

      // If store object becomes empty, delete it
      if (Object.keys(storeObj).length === 0) {
        delete element.storeData[storeId];
      }
    }

    // If storeData becomes empty, delete it
  ...
  if (element?.categories) {
    ...
    for (const level in element.categories) {
      if (Array.isArray(element.categories[level])) {
        // Remove "Not Categorized" values
        element.categories[level] = element.categories[level].filter(
          (category) => category !== "Not Categorized"
        );

        // If category array becomes empty, delete it
        if (element.categories[level].length === 0) {
          delete element.categories[level];
        }
      }
    }

    // If categories object becomes empty, delete it
    if (Object.keys(element.categories).length === 0) {
