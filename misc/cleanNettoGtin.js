function cleanNettoGtin(element) {
  ...
  if (element?.infos) {
    ...
    element.infos = element.infos.filter(info => {
      return Array.isArray(info.items) ? info.items.length > 0 : Object.keys(info.items || {}).length > 0;
    });
  ...
  if (element?.articleId !== undefined && typeof element.articleId === 'string' && element.articleId.includes(';')) {
  ...
  if (element?.searchHierachy && Array.isArray(element.searchHierachy)) {
    element.searchHierachy = element.searchHierachy.filter(item => 
