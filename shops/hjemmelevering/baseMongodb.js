...
class BaseProductDatabase {
  ...
  static async insertAll(batchOrIds, docsArg) {
    ...
    for (let doc of docs) {
      ...
      delete doc.nameURL
      delete doc.nameSEO
      delete doc.promoSplashStyle
      delete doc.promoType
      delete doc._highlightResult
      delete doc.categoriesPIM
      delete doc.categoriesURL
      delete doc.counterATS
      delete doc.inStock
      delete doc.counterATS
      delete doc.lateArrival
      delete doc.productNotification
      delete doc.productNotificationSubHeader
      delete doc.productNotificationType
      ...
      if (this.isFalse(doc?.compareNormalPrice)) {
        delete doc.compareNormalPrice
      }
      if (this.isFalse(doc?.maxPromoQty)) {
        delete doc.maxPromoQty
      }
      if (this.isFalse(doc?.plusDeposit)) {
        delete doc.plusDeposit
      }
      if (this.isFalse(doc?.isOnOffer)) {
        delete doc.isOnOffer
      }
      ...
      if (doc?.ATS) {
        delete doc.ATS.inStock
        delete doc.ATS.inStockDay0
        delete doc.ATS.inStockDay1
        delete doc.ATS.inStockDay2
        delete doc.ATS.nextAvailableDate

        if (this.isFalse(doc?.ATS?.ultraFresh)) {
          delete doc.ATS.ultraFresh;
        }

        if (doc?.ATS?.ultraFresh && !doc?.ultraFresh) {
          doc.ultraFresh = doc?.ATS?.ultraFresh
          delete doc.ATS.ultraFresh
        }
        if (doc?.ATS?.packageSize && !doc?.packageSize) {
      ...
      if (doc?.brand && doc?.targetBrand && doc?.brand === doc?.targetBrand) {
        delete doc.targetBrand
      }

      if (doc?.id && doc?.objectID && doc?.id === doc?.objectID) {
        delete doc.objectID
      }
